 ...
 * org.apache.ibatis.annotations.Update; import
 ...
 * @Mapper
 * 
 * @Repository public interface ResMapper {
 * 
 * @Select("select * from exam") List<Exam> getAllExam();
 * 
 * @Select("select * from exam where id = #{id}") Exam getNExam(int id);
 * 
 * @Insert("insert into exam(id, useremail, userpassword) values(seq_emp.nextVal, #{useremail}, #{userpassword})"
 * ) void insertExam(Exam exam);
 * 
 * @Delete("DELETE FROM exam WHERE id =#{id}") void deleteExam(int id);
 * 
